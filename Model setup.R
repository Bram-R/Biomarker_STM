...
required_packages <- c(
  "docstring", "DiagrammeR", "DiagrammeRsvg", "rsvg", "magick",
...
v_states <- c("TP disease state 1", 
              ...
              "TN disease state 1", 
              "TN disease state 2",
              "TN death")                               # Vector of model health states      
n_states <- length(v_states)                            # Number of health states
v_treatments <- c("Current_practice_with_biomarker",
                  "Current_practice")                   # Vector of strategy names
...
n_p_female <- 0.50                                      # Proportion females

# Country specific
n_setting <- 3 # must be 1 (UK), 2 (FR), or 3 (NL)
