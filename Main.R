...
obj_bcea <- bcea( # create bcea object
  ...
  ref = 1, # selects the 1st row of (e,c) as reference intervention
...
# EVPI population
png(file = paste0("plots/", "population_evpi", ".png"), width = 1500, height = 1500)
plot(
  calc_evpi(
    psa = obj_psa_dam, 
    wtp = v_wtp,
    pop = sum(1 / (1 + df_input$discount_costs[1]) ^ (0:(n_evpi_years - 1)) * n_incidence) # effective population
  ), # calc_evpi end
  currency = n_currency,
  frontier = FALSE,
  points = FALSE,
...
# info rank
df_input_evppi <- df_input[, apply(df_input, 2, var, na.rm = TRUE) != 0] # only select parameters with variance > 0

png(file = paste0("plots/Setting_", n_setting, "_info_rank", ".png"), width = 1500, height = 1500)
info.rank(
  parameter = 1:ncol(df_input_evppi),
  inp = createInputs(inputs = df_input_evppi, print_is_linear_comb = FALSE),
  he = obj_bcea,
  wtp = n_wtp,
  howManyPars = 10,
  graph = "base"
